...
// computeKeyPointsAndDesp 同時提前關鍵點與特徵描述子
...
// estimateMotion 計算兩針之間的運動
// 輸入： 幀1 和 幀2
// 輸出： rvec 和 tvec 旋轉和位移
...
Eigen::Isometry3d cvMat2Eigen(cv::Mat& rvec, cv::Mat& tvec)
{
	...
	cv::Rodrigues(rvec, R);
	Eigen::Matrix3d r;
	//Eigen::Matrix<double, Dynamic, Dynamix> r;
	//cv::cv2eigen(R, r);
	for(int i = 0; i<3; i++)
	{
		for(int j = 0; j<3; j++)
...
// 度量運動的大小 what the hell is this function doing
